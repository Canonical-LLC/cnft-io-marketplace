...
escrowScriptUtxo1=$(scripts/query/escrow.sh | grep $escrowHash1 | head -n 1 | cardano-cli-balance-fixer parse-as-utxo)
escrowScriptUtxo2=$(scripts/query/escrow.sh | grep $escrowHash2 | head -n 1 | cardano-cli-balance-fixer parse-as-utxo)
...
mintValue="-2 $bidMinterId.$escrowScriptHash"

cardano-cli transaction build \
    --alonzo-era \
    $BLOCKCHAIN \
    $(cardano-cli-balance-fixer input --address $buyerAddr $BLOCKCHAIN ) \
    --tx-in $escrowScriptUtxo1 \
    --tx-in-script-file $escrowScriptFile \
    --tx-in-datum-file $escrowDatumFile1 \
    --tx-in-redeemer-file $escrowUnlockRedeemer \
    --tx-in $escrowScriptUtxo2 \
    --tx-in-script-file $escrowScriptFile \
    --tx-in-datum-file $escrowDatumFile2 \
    --tx-in-redeemer-file $escrowUnlockRedeemer \
    --tx-in $utxoScript \
    ...
    --tx-out "$buyerAddr + 10000000 lovelace" \
