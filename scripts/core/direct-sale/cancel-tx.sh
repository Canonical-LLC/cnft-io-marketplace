...
utxoScript=$(scripts/query/direct-sale.sh | grep $datumHash | grep "$value" | head -n 1 | cardano-cli-balance-fixer parse-as-utxo)
scriptHash=$(cat $baseDir/$BLOCKCHAIN_PREFIX/direct-sale.addr)
nftValidatorFile=$baseDir/direct-sale.plutus
redeemerFile=scripts/redeemers/direct-sale/cancel-redeemer.json
